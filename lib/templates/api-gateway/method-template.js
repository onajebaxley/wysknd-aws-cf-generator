...
class MethodTemplate extends Template {
     ...
     * @param {String} [role = '$REGION.<% lambda_invoke_role %>'] An optional
    ...
    setBackendLambda(lambdaFunction, role) {
        ...
        role = role || '$REGION.<% lambda_invoke_role %>';
        const integration = this._ensureProperty('Integration');
        integration.Uri = _lambdaUtils.getLambdaUri(lambdaFunction, '${stageVariables.stack}/invocations');
