...
const mappingTemplateHelper = {
     ...
     */
    mapUserFromJwt: function(map, jwtTokenName, prop) {
        if (!map || map instanceof Array || typeof map !== 'object') {
            throw new Error('Invalid map specified (arg #1)');
        }
        ...
        const rawToken = `$jwt_${_shortId.generate().replace(/-/g, '_')}`;
        const decodedToken = `$decoded_${_shortId.generate().replace(/-/g, '_')}`;
        const jsonToken = `$json_${_shortId.generate().replace(/-/g, '_')}`;

        const mapping = Object.keys(map)
            .map(key => {
                let value = map[key];
                let quotes = '"';
            ...
            /*esfmt-ignore-start*/
                return `\n\t\t"${key}": ${quotes}${jsonToken}['${value}']${quotes}`;
                /*esfmt-ignore-end*/
            })
            .join(',');

    /*esfmt-ignore-start*/
        return `#set(${rawToken} = $input.params('${jwtTokenName}').split('\\.'))
#if(${rawToken}.size() > 2)
#set(${decodedToken} = $util.base64Decode(${rawToken}[1]))
#set(${jsonToken} = $util.parseJson(${decodedToken}))
    "${prop}": {${mapping}
    }
#end`;
        /*esfmt-ignore-end*/
    ...
    mapProperty: function(inputProp, outputProp, options) {
        ...
        const escapeNL = options.escapeNL ? ".replaceAll('\\n','\\\\n')" : '';
        let elseClause = '';

        const inputExpr = `$input.${source === 'json' ? 'path' : source}('${inputProp}')`;
        const outputExpr = `$input.${source}('${inputProp}')`;
        if (typeof options.defaultValue !== 'undefined') {
            const defaultValue = typeof options.defaultValue === 'string' ? options.defaultValue : JSON.stringify(options.defaultValue);
            elseClause = `#else
    ${comma}"${outputProp}": ${quote}${defaultValue}${escapeNL}${quote}
`;
        }

        return `#if(${inputExpr} != "")
    ${comma}"${outputProp}": ${quote}${outputExpr}${escapeNL}${quote}
${elseClause}#end`;
    }
};
