...
class Template {
     ...
     *        depends on. The dependency value is converted into camelcase
     *        unless explicitly overridden by the noCamelCase parameter.
     * @param {Boolean} [noCamelCase=false] If set to true, the dependency
     *        key will be used literally with no transformations.
     *
     * @return {Object} A reference to the template. Can be used to
     *         chain multiple calls.
     */
    addDependency(dependency, noCamelCase) {
        if (typeof dependency !== 'string' || dependency.length <= 0) {
            throw new Error('Invalid dependency specified (arg #1)');
        }
        if (!noCamelCase) {
            dependency = _camelCase(dependency);
        }
