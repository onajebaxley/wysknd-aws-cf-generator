...
class TopicRuleTemplate extends Template {
     ...
     *
     * @param {roleArn} The ARN of the IAM role that grants access to an Kinesis stream.
     *
     * @param {partitionKey} The partition key (the grouping of data by shard within an Kinesis stream).
     *
    ...
    addKinesisAction(streamName, roleArn, partitionKey) {
        ...
        if (typeof roleArn !== 'string' || roleArn.length <= 0) {
            throw new Error('Invalid roleArn - expected a string.');
        }

        if (partitionKey && typeof partitionKey !== 'string') {
            throw new Error('Invalid partitionKey - expected a string.');
        }
     ...
     *
     * @param {state} Setting that detemines of rule is on or off.
     *
     * @return {Object} A reference to the template. Can be used to
     *         chain multiple calls.
     */
     ...
     * Enables the IoT rule
     *
     * @param {sql} The SQL query that will be used by the topic to filter messages.
     *
     * @return {Object} A reference to the template. Can be used to
     *         chain multiple calls.
     */
