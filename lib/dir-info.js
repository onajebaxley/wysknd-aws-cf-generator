...
class DirInfo {
     ...
     */
    get name() {
        return this._pathTokens[this._pathTokens.length - 1];
    }
    ...
    /**
     * Generates a namespaced identifying key for the resource, irrespective of
     * the resource's position in the folder hierarchy. The idea is to
     * encourage developers to use this method when generating resource keys in
     * most cases. The only exception to this rule occures when generating API
     * gateway resources, which leverages a hierarchical directory structure to
     * assign resource keys.
     *
     * @param {String} namespace The namespace to which the resource belongs.
     * @param {String} id The id of the resource for which the token will
     *        be generated.
     * @return {String} The token for the current level.
     */
    getNamespacedToken(namespace, id) {
        if (typeof namespace !== 'string' || namespace.length <= 0) {
            throw new Error('Invalid namespace specified (arg #1)');
        }
        if (typeof id !== 'string' || id.length <= 0) {
            throw new Error('Invalid id specified (arg #2)');
        }
        return this.getRootToken(`${namespace}_${id}`);
    }
